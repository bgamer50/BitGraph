...
int main(int charc, char* argv[]) {
    ...
    try {
        g->addV()->property(NAME, "A")->as("a")->
            ...
            std::cout << "cpu graph created" << std::endl;
            ...
            std::cout << "gpu graph traversal created" << std::endl;
            ...
            h->V()->in()->forEachRemaining([](boost::any& v){   
                Vertex* vtx = boost::any_cast<Vertex*>(v);
                auto id = boost::any_cast<uint64_t>(vtx->id());
                auto label = boost::any_cast<std::string>(vtx->label());
            ...
            h->V()->values(NAME)->forEachRemaining([](boost::any& n){
                std::cout << boost::any_cast<const char*>(n) << std::endl;
            });
