...
GPUGraphStep::GPUGraphStep(bool start, GraphStepType gs_type, std::vector<boost::any> element_ids)
: GraphStep(gs_type, element_ids) {
    this->start = start;
}

void GPUGraphStep::apply(GraphTraversal* trv, TraverserSet& traversers) {
    if(this->start) traversers.push_back(Traverser(boost::any())); // add an empty traverser when this is being used as a start step.
