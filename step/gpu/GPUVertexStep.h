...
class GPUVertexStep : public TraversalStep {
    ...
    public:
        GPUVertexStep(Direction direction, std::vector<std::string> edge_labels, GraphStepType gs_type)
...
void GPUVertexStep::apply(GraphTraversal* traversal, TraverserSet& traversers) {
    ...
    if(this->gs_type == VERTEX) {
        ...
        cudaMemcpyDeviceToHost(new_traversed_vertices.data(), traversed_vertices_device, sizeof(int32_t) * num_new_traversers);
        cudaMemcpyDeviceToHost(originating_traversers.data(), originating_traversers_device, sizeof(int32_t) * num_new_traversers);
        ...
        for(int k = 0; k < num_new_traversers; ++k) {
            ...
            new_traversers[k].get_side_effects().insert(originating_traverser.get_side_effects());
        }
    } else {
