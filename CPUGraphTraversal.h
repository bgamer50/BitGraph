...
class CPUGraphTraversal : public GraphTraversal {
	public:
		...
		: GraphTraversal(src) {
			this->strategies.push_back([this](std::vector<TraversalStep*>& t){bitgraph_strategy(reinterpret_cast<CPUGraph*>(this->getGraph()), t);});
		}
			...
			for(auto index = 0; index < this->steps.size(); index++) {
				switch(this->steps[index]->uid) {
					case GRAPH_STEP: // we implicitly assume this is a Vertex Graph step.
						{
							...
							std::vector<Traverser*>* new_traversers = execute_graph_step(graph_step, traversers);
					...
					case HAS_STEP: {
						...
						std::vector<Traverser*>* new_traversers = this->execute_has_step(has_step, traversers);
					...
					case COALESCE_STEP: {
						...
						for(auto it = traversals.begin(); it != traversals.end(); ++it) {
							...
							std::vector<Traverser*>* temp_traversers = get_all(traversers, &current_traversal);
		...
		*/
		void execute_add_property_step(AddPropertyStep* add_property_step, std::vector<Traverser*>* traversers);

		/*
			Executes the has step.  May be overridden by subclasses (i.e. GPUGraphTraversal)
		*/
		virtual std::vector<Traverser*>* execute_has_step(HasStep* has_step, std::vector<Traverser*>* traversers);

		/*
			Executes the index step.
		*/
		std::vector<Traverser*>* execute_index_step(IndexStep* index_step, std::vector<Traverser*>* traversers);

		/*
			Executes the graph step.
		*/
		std::vector<Traverser*>* execute_graph_step(GraphStep* graph_step, std::vector<Traverser*>* traversers);

		/*
			Executes the graph step (start).
		*/
		void execute_graph_step_start(GraphStep* graph_step, std::vector<Traverser*>* traversers);
		...
		*/
		...
		inline std::vector<Traverser*>* get_all(std::vector<Traverser*>* traversers, CPUGraphTraversal* traversal) {
		...
		*/
		inline boost::any get_next(std::vector<Traverser*>* traversers, CPUGraphTraversal* traversal, bool nocopy) {
			// Call execute in the given traversal with a copy of the current traversers.
			std::vector<Traverser*>* temporary_traversers = nocopy ? traversers : this->copy_traversers(traversers);
			temporary_traversers = traversal->execute(temporary_traversers);
		...
		inline boost::any get_next(std::vector<Traverser*>* traversers, CPUGraphTraversal* traversal) {
			return get_next(traversers, traversal, false);
		}
        ...
        std::vector<Traverser*>* get_initial_traversers() {
			...
			switch(this->steps[0]->uid) {
				...
				case INDEX_STEP: {
					...
					std::vector<Traverser*>* new_traversers = this->execute_index_step(index_step, traversers);
...
void CPUGraphTraversal::execute_add_property_step(AddPropertyStep* add_property_step, std::vector<Traverser*>* traversers) {
	if(add_property_step->get_value().type() == typeid(GraphTraversal*)) {
		...
		/*
		std::vector<std::vector<Traverser*>*> temp_traversers(traversers->size());
		size_t k = 0;
		for(auto it = traversers->begin(); it != traversers->end(); ++it) {
			Traverser* trv = *it;
			temp_traversers[k] = new std::vector<Traverser*>;
			temp_traversers[k]->push_back(new Traverser(trv->get()));
			k++;
		}
		...
		std::vector<boost::any> prop_values(temp_traversers.size());
		size_t sz = temp_traversers.size();
		for(size_t i = 0; i < sz; ++i) {
			CPUGraphTraversal* new_trv = from_anonymous_traversal(traversal);
			boost::any prop_value = this->get_next(temp_traversers[i], new_trv, true);
			//delete temp_traversers[i]->front();
			//delete temp_traversers[i];

			prop_values[i] = prop_value;
		}

		auto it = traversers->begin();
		for(int i = 0; i < traversers->size(); ++i) {
			Traverser* trv = *it;
			++it;

			BitVertex* v = static_cast<BitVertex*>(boost::any_cast<Vertex*>(trv->get()));
			boost::any prop_value = prop_values[i];
...
std::vector<Traverser*>* CPUGraphTraversal::execute_has_step(HasStep* has_step, std::vector<Traverser*>* traversers) {
	std::vector<Traverser*>* new_traversers = new std::vector<Traverser*>;
...
std::vector<Traverser*>* CPUGraphTraversal::execute_index_step(IndexStep* index_step, std::vector<Traverser*>* traversers) {
	std::vector<Traverser*>* new_traversers = new std::vector<Traverser*>;
...
std::vector<Traverser*>* CPUGraphTraversal::execute_graph_step(GraphStep* graph_step, std::vector<Traverser*>* traversers) {
...
void CPUGraphTraversal::execute_graph_step_start(GraphStep* graph_step, std::vector<Traverser*>* traversers) {
	CPUGraph* bg = dynamic_cast<CPUGraph*>(this->getGraph());
