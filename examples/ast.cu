...
int main(int charc, char* argv[]) {
    ...
    for(size_t r = 0; r < tries; ++r) {
        ...
        start = std::chrono::system_clock::now();
        std::vector<std::pair<boost::any, size_t>> count_vec = boost::any_cast<std::vector<std::pair<boost::any, size_t>>>(g->V()->has("INFO","WhileStmt")->as("w")->repeat(in())->emit(has("INFO","IfStmt"))->select("w")->values("NAME")->groupCount()->next());
        end = std::chrono::system_clock::now();
        elapsed = end - start;
